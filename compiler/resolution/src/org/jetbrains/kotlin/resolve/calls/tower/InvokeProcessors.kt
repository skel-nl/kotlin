    ...
    override fun process(data: TowerData): List<Collection<C>> {
        ...
        for (variableCandidates in variableProcessor.process(data)) {
            ...
            if (variableProcessors.isNotEmpty()) {
                if (invokeProcessors.isEmpty()) {
                    previousData.retainAll(this::mayDataBeApplicable)
                }
...
class InvokeTowerProcessor<C : Candidate>(
    ...
    override fun mayDataBeApplicable(data: TowerData) =
            data is TowerData.Empty ||
            (data is TowerData.TowerLevel && data.level.getFunctions(OperatorNameConventions.INVOKE, null).isNotEmpty())
}
...
class InvokeExtensionTowerProcessor<C : Candidate>(
    ...
    override fun mayDataBeApplicable(data: TowerData): Boolean = data is TowerData.Empty || data is TowerData.OnlyImplicitReceiver
}
